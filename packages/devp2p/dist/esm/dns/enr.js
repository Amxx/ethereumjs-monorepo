...
export class ENR {
    ...
    static parseAndVerifyRoot(root, publicKey, common) {
        ...
        const rootVals = sscanf(root, `${this.ROOT_PREFIX}v1 e=%s l=%s seq=%d sig=%s`, 'eRoot', 'lRoot', 'seq', 'signature');
        if (!rootVals.eRoot)
        ...
        const signatureBytes = Uint8Array.from([...base64url.decode(rootVals.signature + '=').values()].slice(0, 64));
    ...
    static parseTree(tree) {
        ...
        const treeVals = sscanf(tree, `${this.TREE_PREFIX}//%s@%s`, 'publicKey', 'domain');
