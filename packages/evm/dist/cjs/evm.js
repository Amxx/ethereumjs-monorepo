...
class EVM {
    constructor(opts = {}) {
        ...
        this.stateManager = opts.stateManager ?? new ethereumjs_statemanager_1.DefaultStateManager();
        // Supported EIPs
        const supportedEIPs = [
            1153, 1559, 2315, 2565, 2718, 2929, 2930, 3074, 3198, 3529, 3540, 3541, 3607, 3651, 3670,
            3855, 3860, 4399, 4895, 4788, 4844, 5133, 5656, 5806, 6780, 6800, 7516,
        ];
    ...
    async _executeCall(message) {
        ...
        if (exit) {
            return {
                execResult: {
                    ...
                    executionGasUsed: ethereumjs_util_1.BIGINT_0,
        ...
        if (message.isCompiled) {
            ...
            if (this._optsCached.profiler?.enabled === true) {
                target = (0, ethereumjs_util_1.bytesToUnprefixedHex)(message.codeAddress.bytes);
    ...
    async _executeCreate(message) {
        ...
        // Check for collision
        if ((toAccount.nonce && toAccount.nonce > ethereumjs_util_1.BIGINT_0) ||
            !((0, ethereumjs_util_1.equalsBytes)(toAccount.codeHash, ethereumjs_util_1.KECCAK256_NULL) === true)) {
            if (this.DEBUG) {
        ...
        if (this.common.gteHardfork(ethereumjs_common_1.Hardfork.SpuriousDragon)) {
            toAccount.nonce += ethereumjs_util_1.BIGINT_1;
        }
        // Add tx value to the `to` account
        let errorMessage;
        try {
        ...
        let returnFee = ethereumjs_util_1.BIGINT_0;
        ...
        if (!result.exceptionError &&
            this.common.gteHardfork(ethereumjs_common_1.Hardfork.SpuriousDragon) &&
        ...
        if (totalGas <= message.gasLimit && (this.allowUnlimitedContractSize || allowedCodeSize)) {
            ...
            if (this.common.gteHardfork(ethereumjs_common_1.Hardfork.Homestead)) {
                if (!allowedCodeSize) {
                    if (this.DEBUG) {
        ...
        if (!result.exceptionError &&
            ...
            if (!this.common.gteHardfork(ethereumjs_common_1.Hardfork.Homestead)) {
                ...
                await this.journal.putAccount(message.to, account ?? new ethereumjs_util_1.Account());
            }
        }
        return {
            createdAddress: message.to,
    ...
    async runInterpreter(message, opts = {}) {
        let contract = await this.stateManager.getAccount(message.to ?? ethereumjs_util_1.Address.zero());
        if (!contract) {
            contract = new ethereumjs_util_1.Account();
        }
        const env = {
            ...
            callValue: message.value ?? ethereumjs_util_1.BIGINT_0,
            code: message.code,
    ...
    async runCall(opts) {
        ...
        if (!message) {
            ...
            this._tx = {
                gasPrice: opts.gasPrice ?? ethereumjs_util_1.BIGINT_0,
            ...
            const value = opts.value ?? ethereumjs_util_1.BIGINT_0;
            if (opts.skipBalance === true) {
                callerAccount = await this.stateManager.getAccount(caller);
                if (!callerAccount) {
                    callerAccount = new ethereumjs_util_1.Account();
                }
        ...
        if (this.DEBUG) {
            ...
            debug(`Received message execResult: [ gasUsed=${executionGasUsed} exceptionError=${exceptionError ? `'${exceptionError.error}'` : 'none'} returnValue=${(0, ethereumjs_util_1.short)(returnValue)} gasRefund=${result.execResult.gasRefund ?? 0} ]`);
        }
        ...
        if (err && err.error !== exceptions_js_1.ERROR.CODESTORE_OUT_OF_GAS) {
            ...
            result.execResult.gasRefund = ethereumjs_util_1.BIGINT_0;
        }
        if (err &&
            !(this.common.hardfork() === ethereumjs_common_1.Hardfork.Chainstart && err.error === exceptions_js_1.ERROR.CODESTORE_OUT_OF_GAS)) {
    ...
    async runCode(opts) {
        ...
        const message = new message_js_1.Message({
            ...
            data: opts.data,
            gasLimit: opts.gasLimit ?? BigInt(0xffffff),
            to: opts.to ?? ethereumjs_util_1.Address.zero(),
            caller: opts.caller,
            value: opts.value,
            depth: opts.depth,
            selfdestruct: opts.selfdestruct ?? new Set(),
    ...
    getPrecompile(address) {
        return this.precompiles.get((0, ethereumjs_util_1.bytesToUnprefixedHex)(address.bytes));
    }
    /**
    ...
    async _generateAddress(message) {
        let addr;
        if (message.salt) {
            addr = (0, ethereumjs_util_1.generateAddress2)(message.caller.bytes, message.salt, message.code);
        }
        else {
            let acc = await this.stateManager.getAccount(message.caller);
            if (!acc) {
                acc = new ethereumjs_util_1.Account();
            }
            const newNonce = acc.nonce - ethereumjs_util_1.BIGINT_1;
            addr = (0, ethereumjs_util_1.generateAddress)(message.caller.bytes, (0, ethereumjs_util_1.bigIntToBytes)(newNonce));
        }
        return new ethereumjs_util_1.Address(addr);
    }
    async _reduceSenderBalance(account, message) {
        account.balance -= message.value;
        if (account.balance < ethereumjs_util_1.BIGINT_0) {
...
EVM.supportedHardforks = [
    ...
    ethereumjs_common_1.Hardfork.Paris,
    ethereumjs_common_1.Hardfork.Shanghai,
    ethereumjs_common_1.Hardfork.Cancun,
    ethereumjs_common_1.Hardfork.Prague,
];
...
function defaultBlock() {
    return {
        header: {
            ...
            gasLimit: ethereumjs_util_1.BIGINT_0,
