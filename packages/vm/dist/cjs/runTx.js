...
async function runTx(opts) {
    ...
    if (opts.tx.supports(ethereumjs_tx_1.Capability.EIP2718TypedTransaction) &&
        ...
        if (opts.tx.supports(ethereumjs_tx_1.Capability.EIP5806Delegate) &&
            this.common.isActivatedEIP(5806) === false) {
            await this.evm.journal.revert();
            const msg = _errorMsg('Cannot run transaction: EIP-5806 is not activated.', this, opts.block, opts.tx);
            throw new Error(msg);
        }
...
async function _runTx(opts) {
    ...
    const isEIP5806 = (0, ethereumjs_tx_1.isDelegateEIP5806Tx)(tx);
    if (this.DEBUG) {
